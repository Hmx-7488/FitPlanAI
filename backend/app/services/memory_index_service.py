...
async def enqueue_missing_memory_upserts(db: AsyncSession) -> int:
    """Backfill index work for pre-outbox active memories without copying content."""
    now = datetime.utcnow()
    existing_job = aliased(UserMemoryIndexOutbox)
    missing = list(
        (
            await db.execute(
                select(UserMemory.id, UserMemory.user_id, UserMemory.index_revision)
                .where(
                    UserMemory.confirmation_status == "confirmed",
                    UserMemory.deleted_at.is_(None),
    ...
    enqueued = 0
    for memory_id, user_id, index_revision in missing:
        result = await db.execute(
            ...
            .values(
                ...
                user_id=user_id,
                operation="upsert",
                index_revision=int(index_revision or 0),
                status="pending",
                attempts=0,
                available_at=now,
            )
        ...
        async with session_factory() as db:
            ...
            try:
                ...
                if job.attempts >= settings.MEMORY_INDEX_MAX_ATTEMPTS:
                    ...
                    job.status = "pending"
                    delay = settings.MEMORY_INDEX_RETRY_BASE_SECONDS * (2 ** (job.attempts - 1))
...
async def run_memory_index_maintenance(
    ...
    *,
    ...
    if not settings.MEMORY_VECTOR_ENABLED:
        return {"processed": 0, "completed": 0, "failed": 0, "obsolete": 0}
    backend = vector_backend or ChromaMemoryVectorStore()
    collection_exists = await backend.collection_exists()
    async with session_factory() as db:
        await enqueue_expired_memory_deletes(db)
        await enqueue_missing_memory_upserts(db)
        if not collection_exists:
            await replay_active_memory_upserts(db)
    return await process_memory_index_outbox(
