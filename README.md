...
基于 LLM + LangGraph + RAG 的个性化减脂/增肌 AI Agent，支持用户建档、营养计算、训练计划生成、食材图片识别、轻食菜谱生成（含 AI 成品图）、餐食热量识别、身材照片分析、AI 动作分析（MediaPipe 关键点 + Vision 解释）、每日打卡、AI 复盘（含数据驱动的热量调整闭环）和带来源引用的上下文聊天助手。知识检索为带来源治理、混合检索和质量评估的统一知识层。后续规划接入 1,324 动作结构化数据集、自建食物热量库和补剂推荐能力，将训练/饮食计划从 LLM 文本升级为结构化 JSON，并将工具注册为 LangGraph Tool 实现 Agent 自主编排。
...
- ~~隐私删除入口~~ ✅ 已实现
- **动作数据集接入**：1,324 动作结构化数据 + GIF 动图 + 中文说明，训练计划从文本升级为结构化
- **食物热量库**：自建 200-300 种中餐食物，餐食识别有数据锚点
- **补剂推荐**：结构化映射 + RAG 知识，按用户目标/伤病推荐
- **训练计划结构化**：后端粗筛 + LLM 精筛编排，伤病安全排除
- **饮食计划结构化**：候选食材池 -> LLM 编排 -> 结构化 JSON
- **Agent 工具化**：数据查询封装为 LangGraph Tool，LLM 自主调用
- 计划调整草案、差异预览和用户确认后写入（训练计划侧）
