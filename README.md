...
基于 LLM + LangGraph + RAG 的个性化减脂/增肌 AI Agent，支持用户建档、营养计算、结构化训练与饮食计划、1,324 项动作数据、自建中餐热量库、补剂推荐、食材图片识别、轻食菜谱生成（含 AI 成品图）、分餐热量识别与每日汇总、身材照片分析、AI 动作分析（MediaPipe 关键点 + Vision 解释）、每日打卡、AI 复盘（含热量和训练计划调整闭环）以及带来源引用的上下文聊天助手。知识检索为带来源治理、混合检索和质量评估的统一知识层；后续重点是把更多数据查询注册为 LangGraph Tool，并继续增强动作分析可靠性。
