# SlimAgent（FitPlanAI）v0.7.1

基于 LLM + LangGraph + RAG 的个性化减脂/增肌 AI Agent，支持用户建档、营养计算、结构化训练与饮食计划、1,324 项动作数据、自建中餐热量库、补剂推荐、食材图片识别、轻食菜谱生成（含 AI 成品图）、分餐热量识别与每日汇总、身材照片分析、AI 动作分析（MediaPipe 关键点 + Vision 解释）、每日打卡、AI 复盘（含热量和训练计划调整闭环）以及带来源引用的上下文聊天助手。知识检索与长期记忆均支持混合检索；长期记忆额外具备确认治理、SQLite 生命周期终检、异步索引一致性、故障降级和实际使用追踪。
...
| POST | `/api/plan/adjust-calories` | 用户确认后写入热量调整 | ✅ 已实现 |
| POST | `/api/plan/adjust-workout` | 校验并写入用户确认的训练调整 | ✅ 已实现 |
...
| AI 复盘 | ✅ 已实现 | 目标类型分支、复盘总结+次日建议+数据驱动的热量调整草案（确认写入） |
| 身材照片分析 | ✅ 已实现 | Vision Model 体脂率估算 + BMI 降级，自动回填档案（置信度≥65%才写入），历史记录可删除 |
...
| 长期记忆混合检索 | ✅ 已实现 | keyword/vector/hybrid + RRF、版本终检、事务 outbox、降级与使用追踪 |
| 隐私删除入口 | ✅ 已实现 | 身材分析记录及照片可删除，上传区有隐私提示 |
| 单用户安全姿态 | ✅ 已实现 | CORS 白名单、非开发环境启动警告、localhost 绑定 |
...
- ~~复盘驱动热量调整闭环~~ ✅ 已实现（草案 -> 确认写入）
- ~~隐私删除入口~~ ✅ 已实现
- ~~**动作数据集接入**：1,324 动作结构化数据 + GIF 动图 + 中文说明~~ ✅ 已实现
- ~~**食物热量库**：自建中餐食物库，餐食识别使用数据锚点~~ ✅ 已实现
- ~~**补剂推荐**：结构化映射 + RAG 知识，按用户目标/伤病/过敏过滤~~ ✅ 已实现
- ~~**训练计划结构化**：后端粗筛 + LLM 精筛编排，伤病安全排除~~ ✅ 已实现
- ~~**饮食计划结构化**：候选食材池 -> LLM 编排 -> 结构化 JSON~~ ✅ 已实现
- ~~**长期记忆混合检索**：独立 Chroma、RRF、SQLite 版本终检、事务 outbox 与实际使用追踪~~ ✅ 已实现
- **Agent 工具化**：数据查询封装为 LangGraph Tool，LLM 自主调用
- ~~训练计划调整草案与用户确认后写入~~ ✅ 已实现（替换类动作会标记待人工选择安全替代动作）
