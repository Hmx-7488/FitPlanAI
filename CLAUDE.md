...
- 接入 1,324 动作结构化数据集（含 GIF + 中文说明），训练计划从 LLM 文本升级为结构化 JSON。
- 自建食物热量库（200-300 种中餐），餐食识别从 100% LLM 估算变为有数据锚点。
- 补剂推荐能力（结构化映射 + RAG 知识）。
- 伤病过滤采用后端粗筛 + LLM 精筛混合方案。
- 将数据查询封装为 LangGraph Tool，从线性管线升级为真 Agent。
- 复盘驱动的热量调整闭环已完成，下一步是训练计划调整闭环。
