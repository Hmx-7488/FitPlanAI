...
| 用户建档 | 目标周期 | target_weeks 字段（PRD §6.2 P0），老库增量迁移 |
...
| 餐食识别 | 热量估算 | Vision Model 识别已吃菜品，估算热量，失败 502 不写假数据 |
| 每日缺口 | 热量汇总 | 当日摄入、剩余配额、缺口状态、调整建议 |
| RAG 知识层 | 专业混合检索 | 13 篇文档，7 域 5 级证据，向量+关键词+重排序 |
...
| AI 复盘 | 分析+建议+热量调整 | 目标类型分支，复盘总结+次日建议+数据驱动的热量调整草案（确认写入） |
| 身材照片分析 | 多证据融合 | 多角度照片 + 围度 + 体脂秤，质量拒绝、结果持久化和历史同角度对比 |
| 身材隐私 | 删除入口 | 记录及照片可删除，上传区有隐私提示（PRD §10.4） |
| AI 动作分析 | MediaPipe + Vision 混合分析 | 33 点关键点、12/18/24 帧采样、角度/轨迹/速度/对称性量化和动作建议 |
| 上下文聊天 | 只读 Agent | 档案/计划上下文注入、流式输出、引用、风险拦截 |
| 安全姿态 | 单用户加固 | CORS 白名单、非开发环境启动警告、localhost 绑定 |
| 前端 | 11 个页面 | 首页、建档、分析、计划、食材、餐食、身材、动作、打卡、复盘、助手 |

### 17.2 当前模型边界与后续增强

| 模块 | 当前实现 | 后续增强 |
| --- | --- | --- |
| 身材照片分析 | Vision Model + 围度公式 + 体脂秤融合，带质量拒绝与历史对比 | 经过标注数据验证的体脂或体态专用模型 |
| AI 动作分析 | 浏览器端 MediaPipe Pose Landmarker + 后端几何量化 + Vision Model 解读 | 动作专用阶段状态机、骨架可视化、Web Worker 和真实视频评测集 |
| 训练计划 | LLM 生成纯文本，无结构化动作引用 | 接入 1,324 动作数据集，结构化 JSON + GIF 动图 |
| 饮食计划 | LLM 生成纯文本，营养数字全靠估算 | 自建食物热量库，结构化 JSON + 数据库锚点 |
| 伤病过滤 | RAG 知识 + prompt 提示 | 后端粗筛 + LLM 精筛混合方案 |
| Agent 编排 | 线性管线，工具硬编码在节点 | 工具封装为 LangGraph Tool，LLM 自主调用 |
| 补剂 | 无 | 结构化映射 + RAG 知识，按目标/伤病推荐 |
...
| 动作分析历史 | 保存每次关键点指标并展示趋势 | P1 |
| 动作骨架可视化 | 视频叠加骨架、关节角度和问题时间点 | P1 |
| 动作专项规则 | 深蹲、硬拉、卧推等动作独立阶段和评分规则 | P1 |

> 数据层接入与 Agent 升级的完整技术设计见 `docs/数据层接入与Agent升级-技术设计文档.md`。
...
加载增量会话摘要与近期消息
-> 识别健康风险
-> qwen 在 8 个只读领域工具中选择最少调用
-> LangGraph 执行预算、去重、超时与安全降级
-> 仅查询模型明确选择的档案、计划、打卡、餐食、食物、动作、知识或长期记忆
-> 对长 RAG/Tool Artifact 微压缩
-> 按统一 Token 预算生成系统上下文
-> DashScope 流式生成
-> 保存正文、引用、工具台账和实际上下文使用状态
```

工具路由成功时不再预加载档案、计划或自动召回长期记忆；仅在工具关闭、Planner/Tool 降级时加载旧上下文兼容链路。
...
- 当前页面名称及前端传入的结构化页面摘要。
- 最新有效会话摘要与预算内的近期聊天消息。
- 已确认、未过期且与当前问题相关的长期记忆。
- RAG Top-K 结果、来源与 Microcompact 诊断。
...
1. M6B：60 条人工标注评测集与内容无关 Agent Run / Tool Run 观测。
2. ~~会话摘要和长期记忆。~~ 已完成 M1-M5.0：增量摘要、覆盖游标、Microcompact、长期记忆治理、混合检索与最小管理入口。
3. M6C：通用变更草案状态机；第一条闭环为聊天热量调整的 Diff、确认和 CAS 写入。
4. ~~工具调用过程可视化。~~ M6A 已完成安全工具台账、来源和实际上下文使用状态。
5. M6D：最小、版本化页面上下文快照，禁止整页状态透传。

### 20.6 上下文与记忆升级（M1-M5.0，2026-08-12）

- [x] Token 预算驱动的统一 Context Builder 与组件级诊断。
- [x] 版本化增量会话摘要、覆盖游标和近期消息尾部。
- [x] RAG/Tool Artifact 独立 Microcompact，保留来源、关键数字与风险约束。
- [x] 仅从用户原话提取长期记忆候选，支持置信度门槛、敏感复核、确认、拒绝、软删除、过期和显式替代。
- [x] 已确认长期记忆接入聊天与计划生成，并记录实际使用的 memory ID。
- [x] 数据库唯一约束、旧库增量迁移、Prompt 指令过滤和非开发环境聊天入口部署密钥保护。
- [x] 独立记忆向量库、keyword/vector/hybrid 三模式、RRF、revision 终检、事务 outbox 与实际使用追踪。
- [x] 固定离线评测：长期记忆 4/4；Microcompact 4/4、构造样例 token 减少 91.9%。这些不是生产流量指标。

主线下一阶段已冻结为 M6B 三模式评测与内容无关观测；M5.1 完整记忆中心作为独立产品增强，不阻塞 M6B。

## 21. 身材与动作分析可靠性升级（v0.9.0）
...
```text
用户上传 1-3 个视角照片
-> Vision Model 逐视角质量检查
-> 不合格时返回 rejected 和重拍指南
-> 合格时融合视觉区间、体脂秤和围度公式
-> 保存照片元数据、测量数据、质量结论和结果
-> 查询同一用户最近有效基线
-> 同角度、相近画幅时调用二次视觉比较
-> 返回阶段变化与测量差值
```
...
- `BodyAnalysis` 数据表。
- `body_analysis_service.py`：测量范围校验、US Navy 围度公式、质量判定、证据融合和可比视角筛选。
- `GET /api/body/history/{user_id}` 历史接口。
- 自动回填阈值由 45% 提升到 65%，并禁止降级结果自动回填。

前端新增：

- 腰围、臀围、胸围、颈围、体脂秤和本次体重录入。
- 逐视角质量、拒绝原因、重拍指南、估算证据和阶段对比。
- 历史记录列表与照片放大。
...
- 使用 `@mediapipe/tasks-vision` Pose Landmarker Lite。
- 模型与 WASM 放在 `frontend/public/mediapipe/`，运行时不依赖外部 CDN。
- GPU 初始化失败时自动回退 CPU。
- 视频按时长采样：不超过 6 秒取 12 帧，6-15 秒取 18 帧，超过 15 秒取 24 帧。
- 全部采样帧用于关键点量化，最多 6 个代表帧发送给 Vision Model。

后端量化：

- 计算膝、髋、肘、肩关节角度的最小值、最大值、均值和变化范围。
- 计算左右关节平均角度差和对称性评分。
- 根据髋部中心轨迹计算速度控制与轨迹平滑度。
- 根据垂直方向变化估算重复次数。
- 根据关键点可见度、有效帧比例和人物画面边缘比例判断拍摄质量。

结果来源：

| 来源 | 含义 | 用户界面 |
| --- | --- | --- |
| `hybrid` | 关键点量化 + Vision Model 解读 | 关键点量化 + AI 解读 |
| `keypoint_only` | Vision 失败，关键点计算有效 | 仅关键点量化 |
| `vision_only` | 未形成稳定动态量化 | 仅视觉估算 |
| `template` | 两种真实分析均失败 | 模板参考，并显示非真实检测警告 |
| `rejected` | 拍摄质量不足 | 拒绝评分并给出重拍建议 |

### 21.3 验证基线

- 后端单元和回归测试：106 项通过。
- 前端 Vue 类型检查与 Vite 生产构建通过。
- MediaPipe 模型文件和 WASM 本地访问返回 HTTP 200。
- 动作与身材页面在 1440px 桌面和 390px 移动端无横向溢出。
- 当前分支提交：
  - `376c36f feat: improve body analysis reliability`
  - `8422578 feat: add quantitative pose analysis`

### 21.4 下一阶段

1. 为每个动作建立专项阶段状态机和评分阈值。
2. 保存动作分析记录，支持同动作历史趋势。
3. 在视频上叠加骨架、角度和问题时间轴。
4. 将 MediaPipe 推理迁移到 Web Worker，减少长视频处理时的界面阻塞。
5. 建立授权真实视频评测集，统计关键点覆盖、次数误差、角度误差和拒绝准确率。
6. 将动作纠正和身材建议接入 RAG，并展示专业来源。

## 22. M5.0 长期记忆混合检索（2026-08-12）

长期记忆召回已从单一规则排序升级为 `keyword / vector / hybrid` 三模式，默认使用 hybrid：

- SQLite 保持唯一业务真值，向量命中必须回查用户、确认状态、删除状态、有效期、fingerprint 和 revision。
- 长期记忆使用独立 `memory_vectorstore`，不与知识 RAG 的目录生命周期互相影响。
- hybrid 使用 RRF 融合，不直接比较关键词分与向量相似度的不同量纲。
- 记忆状态与 outbox 在同一事务提交；embedding/Chroma 写入异步、幂等、可退避重试。
- 编辑和删除先在 SQLite 立即生效，旧向量即使残留也无法通过版本终检。
- 聊天和计划记录检索通道、排名、降级原因与实际进入上下文的记忆，且不持久化原始 query。
- Chroma、embedding 或索引不可用时显式降级 keyword，主业务仍可工作。

完整实现与面试说明见 [[上下文与记忆-M5-混合检索实现记录]]。

下一阶段 M6B 将在 60 条人工标注固定数据集上比较三种模式的 Recall@K、MRR、硬约束漏召回率、延迟和 embedding 成本，并基于指标调节候选窗口与 RRF 参数。
...
## 24. M6B 评测与内容无关观测（方案冻结于 2026-08-13）

M6B 先于 Agent 写操作开发。首版冻结为 60 条人工标注用例，分层评测 Tool 选择、三模式长期记忆检索、上下文事实可见性、摘要保持、引用支持、健康安全、故障降级、延迟、Token 和成本。

生产观测新增 Agent Run / Tool Run 概念，只保存版本、状态、耗时、Token、成本、计数、引用 ID 和稳定错误码；不保存原始问题、Prompt、Tool 参数/返回正文、记忆正文、图片或视频内容。CI 使用确定性评测，真实 qwen 基线显式运行并保存不可覆盖的版本化报告。

M6B 后进入 M6C：先实现通用 `AgentChangeDraft` 状态机，再复用现有热量调整规则完成聊天中的“草案 → Diff → 用户确认 → CAS 写入”。M6D 最后接入最小、版本化页面快照。完整设计见 [[聊天Agent-M6B-评测与观测设计]]。
