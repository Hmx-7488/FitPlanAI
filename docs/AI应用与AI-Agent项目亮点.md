...
状态：✅ M6A 只读 Tool-Calling 已实现；🧊 M6B 方案已冻结并进入开发
...
- 首版建立 60 条人工标注用例，覆盖无工具、单/多工具、三模式记忆检索、长上下文、安全注入和故障降级。
- 固定比较 keyword/vector/hybrid 的 Recall@K、MRR、硬约束漏召回率、延迟、降级率和 embedding 成本，先形成基线再调节 RRF 与候选窗口。
- 建立内容无关 Agent Run / Tool Run 观测，不保存原始问题、Prompt、Tool 正文或记忆正文。
- 分别量化工具选择、上下文事实可见性、摘要保持、引用支持、安全遵守、延迟、Token 和成本。
- CI 使用确定性评测；真实 qwen 评测显式触发并保存带数据集、模型和 Prompt 版本的不可覆盖报告。

完整冻结方案见 [[聊天Agent-M6B-评测与观测设计]]。M6B 完成后进入 M6C：先实现通用变更草案状态机，再以聊天热量调整完成第一条“草案 → Diff → 确认 → CAS 写入”闭环；M6D 最后补最小版本化页面快照。
...
- 2026-08-13：冻结 M6B-M6D 路线；M6B 首版采用 60 条人工标注评测集与内容无关观测，M6C 第一条写闭环固定为聊天热量调整，M6D 使用最小版本化页面快照。
